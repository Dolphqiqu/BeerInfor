...
class SearchViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var BeerAndBrewery: UISegmentedControl!
    ...
    internal func fetchDataFromBreweryName(name: String) {
        ...
        let urlString = "https://sandbox-api.brewerydb.com/v2/search?q=\(newName)&type=brewery&withBreweries=Y&withGuilds=Y&key=3005decedadec80f1a27659b3699692f"
    ...
    internal func fetchDataFromBeerName(name: String) {
        let newName = name.replacingOccurrences(of: " ", with: "%20").lowercased()
        let urlString = "https://sandbox-api.brewerydb.com/v2/search?q=\(newName)&type=beer&key=3005decedadec80f1a27659b3699692f"
        if let url = URL(string: urlString) {
            let request = URLRequest(url: url)
            ...
            let task = URLSession.shared.dataTask(with: request) {
                ...
                if error == nil {
                    ...
                    do {
                        ...
                        let beerName = reply.product?.generic_name_en
                        if beerName != nil {
